...
class ProductController extends Controller {
     ...
     public function productdatatable(){
         ...
         return DataTables::of($category)
            ...
            ->editColumn('action', function ($category) {                 
                 ...
                 $return = '<a  href="'.$editoption.'" rel="tooltip"  class="m-b-10 m-l-5" data-original-title="Remove"><i class="fa fa-edit f-s-25" style="margin-right: 10px;font-size: x-large;"></i></a>
                 <a onclick="delete_record(' . "'" . $deletecatlog. "'" . ')" rel="tooltip"  class="m-b-10 m-l-5" data-original-title="Remove" style="margin-right: 10px;"><i class="fa fa-trash f-s-25" style="font-size: x-large;"></i></a>
                 <a href="'.$changestaus.'" rel="tooltip" class="m-b-10 m-l-5" data-original-title="Remove" style="margin-right: 10px;"><i class="fa fa-ban f-s-25" style="font-size: x-large;color:'.$color.'"></i></a>
                 <a  onclick="previewproduct('.$category->id.')" rel="tooltip"  class="m-b-10 m-l-5" data-original-title="Remove" data-toggle="modal" data-target="#previewproduct"><i class="fa fa-eye f-s-25" style="margin-right: 10px;font-size: x-large;"></i></a>';
                 return $return;              
            })           
            ->make(true);
     }
     ...
     public function saveproduct(Request $request){
        ...
        $store->author = $request->get("author");
        $store->language = $request->get("language");
        $store->publisher = $request->get("publisher");
        $store->isbn = $request->get("isbn");
        $store->binding=$request->get("binding");
        $store->publish_date=$request->get("publish_date");
        $store->weight=$request->get("weight");
        $store->product_edition=$request->get("product_edition");
        $store->dimension=$request->get("dimension");
        $store->print_on_demand=$request->get("print_on_demand");
        $store->save();
     ...
     public function savevender(Request $request){
        if($request->get("product_id")!=0){
            $store=Product::find($request->get("product_id"));
        }
        else{
        ...
        $store->vender_name = $request->get('vender_name');
        $store->save();
     ...
     public function savedeliverycharges(Request $request){
        ...
        $store->delivery_days = $request->get('delivery_days');
        $store->save();
        return redirect('admin/savecatalog/'.$store->id.'/6');
     }
     ...
     public function saveinventory(Request $request){
        ...
        if($request->get("sku")==""){
            $store=Product::find($request->get("product_id"));
                ...
                $store->save();
                return redirect('admin/savecatalog/'.$store->id.'/7');
        }else{
     ...
     public function saveproductimage(Request $request){
        ...
        return redirect('admin/savecatalog/'.$store->id.'/8');
     }
     ...
     public function saveproductattibute(Request $request){
       // dd($request->all());exit;
        if($request->get("product_id")==0){
            Session::flash('message',__('messages_error_success.general_form_msg')); 
            Session::flash('alert-class', 'alert-danger');
            return redirect('admin/savecatalog/0/8');
        }
        $arr=array_values($request->get("attributeset"));
        if(count($arr)==0){             
             return redirect('admin/savecatalog/'.$store->id.'/9');
        }
     ...
     public function saveproductoption(Request $request){
         ...
         if($request->get("product_id")==0){
            Session::flash('message',__('messages_error_success.general_form_msg')); 
            Session::flash('alert-class', 'alert-danger');
            return redirect('admin/savecatalog/0/9');
        }
        if($request->get("totaloption")==0){    
            return redirect('admin/savecatalog/'.$request->get("product_id").'/8');
        }
        ...
        $price=array();
         ...
         $store->save();
          return redirect('admin/savecatalog/'.$request->get("product_id").'/10');
  ...
  public function saverealtedprice(Request $request){
     ...
     if($request->get("product_id")==0){
            Session::flash('message',__('messages_error_success.general_form_msg')); 
            Session::flash('alert-class', 'alert-danger');
            return redirect('admin/savecatalog/0/10');
        }
        if($request->get("totaloption")==0){
        ...
        if($id=="0"){
         ...
         return DataTables::of($review)
            ...
            ->editColumn('action', function ($attribute) { 
                 ...
                 <a  href="'.$editoption.'" rel="tooltip"  class="m-b-10 m-l-5" data-original-title="Remove"><i class="fa fa-edit f-s-25" style="margin-right: 10px;font-size: x-large;"></i></a>';
                 return $return;              
            })           
            ->make(true);
     }
    ...
    public function export(){
        return Excel::download(new BulkExport, 'book_factory.xlsx');
    }
}