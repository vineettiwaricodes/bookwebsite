...
class BannerController extends Controller {
   ...
   public function updatebanner(Request $request){
     ...
     return view("admin.promobanner.default")->with("img1",$img1)->with("img2",$img2)->with("bannerdata",$banner);
     }

     public function updatepromobanner(Request $request){
      if ($files = $request->file('photo1')) {
              $file = $request->file('photo1');
              $filename = $file->getClientOriginalName();
              $extension = $file->getClientOriginalExtension() ?: 'png';
              $folderName = '/upload/promobanner/image/';
              $picture = str_random(10).time() . '.' . $extension;
              $destinationPath = public_path() . $folderName;
              $request->file('photo1')->move($destinationPath, $picture);
              $img_url =$picture;
              $photo =PromoBanner::find(1);
            if(empty($photo)){
               $photo=new PromoBanner();
            }
            $photo->Image = $img_url;
            $photo->title=$request->get("title1");
            $photo->description=$request->get("description1");
            $photo->position='1';
            $photo->save();
     }
     else{
     ...
     if ($files = $request->file('photo2')) {
              ...
              $request->file('photo2')->move($destinationPath, $picture);
              $img_url =$picture;
            $photo =PromoBanner::find(2);
            if(empty($photo)){
               $photo=new PromoBanner();
            }
            $photo->Image = $img_url;
            $photo->title=$request->get("title2");
            $photo->description=$request->get("description2");
            $photo->position='2';
            $photo->save();
     }
     else{
